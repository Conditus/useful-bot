...
def getEncryptedValue(keyName):
    with open(keyName + ".privatekey", mode = 'rb') as privatefile:
        keydata = privatefile.read()
    PrivateKey = rsa.PrivateKey.load_pkcs1(keydata)
    with open(keyName + ".publikkey", mode = 'rb') as pubfile:
        keydata = pubfile.read()
    PublicKey = rsa.PublicKey.load_pkcs1(keydata)
    with open(keyName + ".encoded", "rb") as keyFile:
        key = str(rsa.decrypt(keyFile.read(), PrivateKey).decode())
    return key


print(getEncryptedValue("confirmationToken"))
print(getEncryptedValue("secretKey"))
